...
Expected Move는 방향 적중과 별도 calibration 지표다.
...
`AVOID_FAILURE`: 피하라고 한 종목이 FE ≥ 5%를 기록하여 큰 단타기회를 놓친 경우.
`AVOID_SUCCESS`: 피하라고 한 종목이 FE < 3%이거나 큰 AE/약한 종가 등으로 회피 논리가 유효했던 경우.
3~5% 또는 논리가 혼합된 경우 `MIXED`.

AVOID 성과는 TOP PICKS KPI와 분리한다.

## 18. Market-wide Capture — 가능할 때만
...
## 21. Observation / Improvement Candidate
...
BEST_MODEL_TODAY는 한 지표가 아닌 Big-Move KPI 전체를 종합해 선택한다.
명확한 우위가 없으면 `MIXED`.
...
STAGE_TIME|Rank|Name|Code|Market|ExpectedMoveFE|Confidence|PrevClose|Open|High|Low|Close|C2C|O2C|FE|AE|FE3Hit|FE5Hit|FE10Hit|ExpectedMoveResult|SuccessFactor|ErrorClass|ErrorSubtype

### DEEP 결과행
`RESULTS_DEEP`에서 사용:

STAGE_TIME|Rank|Name|Code|Market|ExpectedMoveFE|BaseScore|RawScore|AvailableMax|Coverage|Breakdown_C/I/D/L/E/R/Q|Confidence|PriceInCheck|PriceIn|ChaseRisk|OpenExpectation|OpenActual|OpenHit|PrevClose|Open|High|Low|Close|C2C|O2C|FE|AE|FE3Hit|FE5Hit|FE10Hit|ExpectedMoveResult|SuccessFactor|ErrorClass|ErrorSubtype

NOT_COMPARABLE도 결과행에는 남기되 집계에서는 제외한다.

## 24. REVIEW CAPSULE — 장기 축적용
...
- Market Capture는 전체 KRX 모집단을 신뢰성 있게 확보한 경우에만 계산한다.
